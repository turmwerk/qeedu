...
import {
  processAssistantPageData,
  processAssistantQuickActions,
...
const laneOrder = ["进行中", "阻塞中", "已完成"];

const ProcessAssistant: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { caseId } = useParams();
  const { records, createRecord, patchRecord, removeRecord } = useFeatureRecords(
    processAssistantAdapter,
  );
  ...
  const selected = records.find((record) => record.id === caseId) ?? filtered[0] ?? records[0] ?? null;
  ...
  return (
    <div className="flex w-full min-w-0 flex-col gap-6 px-4 py-6 md:px-6 xl:px-8">
      <section className="rounded-[34px] bg-[linear-gradient(135deg,#0f172a_0%,#1d4ed8_48%,#38bdf8_100%)] p-6 text-white shadow-[0_24px_60px_rgba(30,64,175,0.28)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            <div className="text-xs font-bold uppercase tracking-[0.24em] text-cyan-100/80">
              Management Flow Studio
            </div>
            <div className="mt-3 text-3xl font-black md:text-4xl">
              {processAssistantPageData.headline}
            </div>
            <div className="mt-3 text-sm leading-7 text-cyan-50/90">
              {processAssistantPageData.description}
            </div>
          </div>
          <div className="flex gap-3">
            <Button
              ...
              onClick={() => navigate("/management/process-assistant")}
            >
              案例总览
            </Button>
            <Button
              className="rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-slate-900"
              onClick={() => navigate("/management/process-assistant/cases/new")}
            >
              新建案例
            </Button>
          </div>
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white/85 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-white/6">
              ...
              <div className="space-y-3">
                {lane.items.map((item) => (
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => navigate(`/management/process-assistant/cases/${item.id}`)}
                  ...
                  >
                    ...
                    <div className={`mt-1 text-xs ${selected?.id === item.id ? "text-slate-200" : "text-slate-500 dark:text-slate-300"}`}>
                      {item.subtitle}
                    </div>
      ...
      {selected ? (
        <section className="grid gap-6 2xl:grid-cols-[minmax(0,1.18fr)_360px]">
          <div className="space-y-6">
            <div className="rounded-[30px] border border-slate-200 bg-white/90 p-5 shadow-[0_20px_44px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-white/6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    variant="secondary"
                    onClick={() => {
                      updateSelected({ status: "已完成", updatedAt: Date.now() });
                      showToast("案例已标记为完成");
                    }}
                  >
                    标记完成
                  ...
                  <Button
                    variant="primary"
                    onClick={() => showToast("AI 已重新生成办理步骤")}
                  >
                    AI 生成步骤
                  </Button>
                </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
              <ChecklistBoard tasks={selected.tasks ?? []} onToggle={(taskId) => {
                updateSelected({
            ...
            <div className="rounded-[30px] border border-slate-200 bg-slate-950 p-4 text-white shadow-[0_24px_52px_rgba(15,23,42,0.24)]">
              <ChatDialog
                dialogId={`management-process-${selected.id}`}
                botName="流程纠偏助手"
                initMessage="我已经读取当前案例、步骤清单和异常信息，可以继续帮你重排步骤、生成补救动作或整理提醒。"
                transport={processAssistantAdapter.createChatTransport(selected.title)}
              />
            </div>
          </div>
