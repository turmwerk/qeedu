...
export function showToast(message: React.ReactNode, duration = 3000) {
  window.dispatchEvent(
    new CustomEvent("nju-show-toast", { detail: { message, duration } }),
  );
}
...
const ToastContainer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("nju-show-toast", handler as EventListener);
  }, []);
  ...
  return ReactDOM.createPortal(
    <div
      className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-2 z-[9999] pointer-events-none items-center justify-center"
      data-oid="x242daw"
    >
      <style data-oid="mbof6n:">{`
        @keyframes toastIn {
