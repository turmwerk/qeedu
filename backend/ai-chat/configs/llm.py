...
- default chat/fix model: a fast free OpenRouter model
- DeepSeek official models remain available when selected explicitly
...
DEFAULT_CHAT_MODEL: str = os.getenv(
    "LLM_CHAT_MODEL", _chat_cfg.get("model", "liquid/lfm-2.5-1.2b-instruct:free")
).strip()

# Backward-compatible exports for package-level re-exports in configs/__init__.py.
LLM_PROVIDER: str = "deepseek" if DEFAULT_CHAT_MODEL.startswith("deepseek-") else "openrouter"
